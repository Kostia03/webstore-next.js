# LIVE Sale Pro Storefront

## Getting Started

1. `cp .env.example .env` - Creates `.env` based on `.env.example`
2. `ln -s docker-compose.dev.yml docker-compose.override.yml` - Symlinks `docker-compose.override.yml` to `docker-compose.dev.yml`
3. `docker-compose up` - Runs in dev mode
